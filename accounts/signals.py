...
def create_user_profile(sender, instance, created, raw, **kwargs):  # Функция, которая будет вызываться после сохранения пользователя
    ...
    if created:  # Проверяем, был ли пользователь только что создан
        Profile.objects.get_or_create(user=instance)  # Создаем профиль для пользователя, если его еще нет